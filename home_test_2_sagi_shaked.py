...
def return_info(flight_id):
    flights_data = pd.read_csv('after_q1_flights.csv')
...
def add_flight():
    ...
    if len(missed_keys) != 0:
        ...
        flights_data.to_csv('after_q1_flights.csv', index=False)
...
def remove_flight(flight_id):
    flights_data = pd.read_csv('after_q1_flights.csvv')
    ...
    if flights_data.equals(flights_data_after_drop):
        ...
        flights_data_after_drop.to_csv('after_q1_flights.csv', index=False)
